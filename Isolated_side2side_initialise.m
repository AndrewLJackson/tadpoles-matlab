...
%
% ********************************************************************
% WARNING - AND NB. Andrew Jackson 29-June-2006
% Only the circular shoal actually isolates the focal prey.
% ********************************************************************
...
if length(shape) == 1 % circular group
    ...
    if pick_side < 3
        ...
        C(2,1) = C(2,1) + iso.*sign(C(2,1)); 
        C(1,1) = mean(C(1,:));
        C(1,:) = C(1,:) + sign(axlims(pick_side)).*(r+1) + (axlims(pick_side));
        C(2,:) = C(2,:) + axlims(1) - 1 + (r+iso) + (2.*axlims(2)-2-(2.*(r+iso))).*rand;
        V = unitvector([ -ones(1,n).*sign(axlims(pick_side)) ; zeros(1,n) ]) ;
    else
        [tmp Xextreme] = max(C(1,:));
        C(1,1) = C(1,1) + iso.*sign(C(1,1));  
        C(2,1) = mean(C(2,:));
        C(2,:) = C(2,:) + sign(axlims(pick_side)).*(r+1) + (axlims(pick_side));
        C(1,:) = C(1,:) + axlims(3) - 1 + (r+iso) + (2.*axlims(4)-2-(2.*(r+iso))).*rand;
