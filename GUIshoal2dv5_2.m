...
%1) countdown at the start of each run

function GUIshoal2dv5_2%(n,Rr,Ro,Ra,aa,v,dtheta,sigma,trials);
global xy C GotOne clickcount trial trials txtcntdwn txt1%these variable are available to all functions in the workspace
...
n = 50; %number of individuals in the shoal
...
v = 3 .*ones(1,n).*0.1;        %speed in units per 0.1sec (per iteration)
...
figure(1)
clf
set(1,'NextPlot','ReplaceChildren','color',[0.9 0.9 0.9])
...
%these are the push buttons on the bottom left of the screen and the text
%count down
txtcntdwn = [];
txt1 = text(0,0,num2str(txtcntdwn));
pb1 = uicontrol('Style', 'pushbutton', 'String', 'GO AGAIN',...
    'Position', [10 10 100 40],'units','pixels', 'Callback', 'GUIshoal2dv5_2');
pb2 = uicontrol('Style', 'pushbutton', 'String', 'STOP',...
    'Position', [10 60 100 40],'units','pixels', 'Callback', 'stop');

for i = 5:-1:0 % this is the value you change to give a longer or shorter countdown
    pause(1)
    txtcntdwn = i;
